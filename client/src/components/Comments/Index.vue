<template>
    <div>
        ...
        <h4>จำนวน ความคิดเห็น {{comments.length}}</h4>
        <div v-for="comment in comments" v-bind:key="comment.id">
            <p>id: {{ comment.id }}</p>
            <p>blog id: {{ comment.blogId }}</p>
            <p>ความคิดเห็น: {{ comment.comment }}</p>
            <p>
                <button v-on:click="navigateTo('/comment/'+ comment.id)">ดูความคิดเห็น</button>
                <button v-on:click="editComment('/comment/'+ comment.id)">แก้ไขข้อมูล</button>
